...
public final class MXCommandHandler implements TabExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (!commandSender.hasPermission(X.permission)) {
            commandSender.sendMessage("You don't have permission!");
            return true;
        }
        if (!label.equalsIgnoreCase(X.command)) {
            commandSender.sendMessage("Usage: /" + X.command);
            return true;
        }
    ...
    private void showHelps(CommandSender sender) {
        sender.sendMessage(wrapColors("&9&l" + X.name + " &fCommands"));
        sender.sendMessage("");
        for (MXSubCommand subCommand : subCommands) {
            ...
            String message = wrapColors(ChatColor.YELLOW + "/" + X.command + " " + subCommand.getName() + " " + ChatColor.WHITE + "- " + ChatColor.RED + subCommand.getDescription());
            if (sender instanceof Player) {
                TextComponent textComponent = new TextComponent(message);
                textComponent.setClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/" + X.command + " " + subCommand.getName()));
