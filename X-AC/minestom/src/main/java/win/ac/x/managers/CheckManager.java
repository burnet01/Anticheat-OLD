...
import win.ac.x.checks.v2.movement.simulation.SimulationBV2;
import win.ac.x.checks.v2.movement.simulation.SimulationCV2;
import win.ac.x.checks.v2.movement.simulation.SimulationDV2;
...
public class CheckManager {
    ...
    public void init() {
        instances.clear();
        File dataFolder = XMinestom.getDataFolder().toFile();
        ...
        for (Class<? extends PacketCheckHandler> handlerClass : checks) {
            ...
            Map<String, Object> section = (Map<String, Object>) configMap.computeIfAbsent(sectionName, k -> new LinkedHashMap<>());

            for (Map.Entry<String, Object> e : defaultParams.entrySet()) {
                String key = e.getKey();
                Object val = e.getValue();
                if (val instanceof Map) {
                    section.computeIfAbsent(key, k -> new LinkedHashMap<>((Map<?, ?>) val));
                } else {
                    section.putIfAbsent(key, val);
                }
            }
            ...
            for (Map.Entry<String, Object> e : defaultParams.entrySet()) {
                ...
                if (val instanceof Map) {
                    Map<?, ?> sub = (Map<?, ?>) section.get(key);
                    mergedParams.put(key, sub != null ? new LinkedHashMap<>(sub) : new LinkedHashMap<>());
                } else {
    ...
    public Map<String, Object> getConfig(Class<?> clazz) {
        PacketCheckHandler handler = instances.get(clazz.getName());
        return handler != null ? handler.getConfig() : new HashMap<>();
    }
}