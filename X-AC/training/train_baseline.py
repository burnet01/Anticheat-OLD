...
Segments into [40, 4] windows: [yawDelta, pitchDelta, hspeed, clickInterval]
Trains a 1D-CNN + BiLSTM classifier, exports to baseline_{label}.onnx

Usage:
  python train_baseline.py                       # train on ALL cheat labels vs legit
  python train_baseline.py --label killaura      # train only on "killaura" vs legit
  python train_baseline.py --label scaffold      # train only on "scaffold" vs legit
"""
...
NOISE_FRACTION = 0.04       # gaussian noise as fraction of feature std
SENS_SCALE_RANGE = (0.80, 1.25)  # simulate different mouse sensitivity
TIME_WARP_RANGE = (0.92, 1.08)  # simulate different click speeds
...
OUTPUT_DIR  = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "plugins", "MX", "models"))
...
def load_sessions_by_file(directory, target_label=None, target_letter=None):
    ...
    for fp, is_cheat in all_files:
        ...
        min_len = min(len(yaws), len(pitches), len(hspeeds))
        if min_len < WINDOW:
            continue
        ...
        if len(clicks) == 0:
            click_aligned = [0.0] * min_len
        else:
            click_aligned = []
            ratio = (len(clicks) - 1) / max(1, min_len - 1)
            for i in range(min_len):
                idx = int(i * ratio)
                idx = min(idx, len(clicks) - 1)
                click_aligned.append(float(clicks[idx]))
...
def augment_window(window, label, rng):
    ...
    # 1. Gaussian noise on every feature (scaled per-feature by its own std)
    for f in range(FEATURES):
        col = w[:, f]
        std = max(np.std(col), 1e-6)
        col += rng.normal(0, NOISE_FRACTION * std, size=col.shape).astype(np.float32)

    # 2. Sensitivity scaling — simulates different mouse DPI/sensitivity
    #    yaw/pitch deltas scale together
    sens = rng.uniform(*SENS_SCALE_RANGE)
    w[:, 0] *= sens   # yawDelta
    w[:, 1] *= sens   # pitchDelta

    # 3. Speed scaling — simulates server TPS variation
    w[:, 2] *= rng.uniform(*SPEED_SCALE_RANGE)

    # 4. Click interval time-warp — simulates different click speeds
    time_warp = rng.uniform(*TIME_WARP_RANGE)
    w[:, 3] *= time_warp
...
def main():
    ...
    parser.add_argument("--label", type=str, default=None, help="Cheat label to train on (e.g. killaura, scaffold)")
    ...
    output_file = os.path.join(OUTPUT_DIR, f"baseline_{label_name}{model_suffix}.onnx")
    ...
    print(f"  Deploy: place at plugins/MX/models/baseline_{label_name}{model_suffix}.onnx")
